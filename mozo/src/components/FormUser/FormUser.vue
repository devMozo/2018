<template>
    <section :id="getHashCode()">
        <form @submit.prevent="refresh()" class="materia-shadow">
            ...
            <FormUserField :title="'Gender'">
              <FilterSexUser v-on:updateSex="updateActiveSex"></FilterSexUser>
            </FormUserField>
...
import FormUserField from '@/components/FormUser/FormUserField'
...
export default {
  ...
  computed: {
    id: function () {
      let countUsers = JSON.parse(window.localStorage.getItem('people')) || []
  ...
  methods: {
    ...
    validate () {
      if (this.user.name === '' || this.user.age < 0 || this.user.sex === '') { return false }
